...
function compEncDecDecompress {
  ...
  case $1 in # Time
  "gzip" | "bzip2")
    (time $cCmd <$2 >$in.$cFT) &>$result_FLD/${upInComp}_${upInEnc}_CT__${inwf}_$ft
    ;;
  ...
  "fqc")
    (time $cCmd -i $2 -o $in.$cFT) &>$result_FLD/${upInComp}_${upInEnc}_CT__${inwf}_$ft
    ;;

  "mfcompress")
    (time $cCmd -o $in.$cFT $2) &>$result_FLD/${upInComp}_${upInEnc}_CT__${inwf}_$ft
    ;;
  esac

  ls -la $in.$cFT >$result_FLD/${upInComp}_${upInEnc}_CS__${inwf}_$ft       # Size
  progMemoryStop $MEMPID $result_FLD/${upInComp}_${upInEnc}_CM__${inwf}_$ft # Mem
  ...
  progMemoryStop $MEMPID $result_FLD/${upInComp}_${upInEnc}_EnM__${inwf}_$ft # Mem
  ...
  progMemoryStop $MEMPID $result_FLD/${upInComp}_${upInEnc}_DeM__${inwf}_$ft # Mem
  ...
  case $1 in # Time
  "gzip" | "bzip2" | "lzma")
    (time $dCmd <$encPath/$in.$cFT >$in) &>$result_FLD/${upInComp}_${upInEnc}_DT__${inwf}_$ft
    ;;
  ...
  "fqc")
    (time $dCmd -i $encPath/$in.$cFT -o $in) &>$result_FLD/${upInComp}_${upInEnc}_DT__${inwf}_$ft
    ;;

  "mfcompress")
    (time $dCmd -o $in $encPath/$in.$cFT) &>$result_FLD/${upInComp}_${upInEnc}_DT__${inwf}_$ft
    ;;
  esac

  progMemoryStop $MEMPID $result_FLD/${upInComp}_${upInEnc}_DM__${inwf}_$ft # Mem
...
function compEncDecDecompOnDataset {
  ...
  if [[ ! -d $progs/$methodComp ]]; then
    mkdir -p $progs/$methodComp
  fi
  if [[ ! -d $progs/$methodEnc ]]; then
    mkdir -p $progs/$methodEnc
  fi
  dsPath="../../$dataset"

  case $2 in
  "fa" | "FA" | "fasta" | "FASTA") # FASTA -- human - viruses - synthetic
    compEncDecDecompress $methodComp $dsPath/$FA/$HUMAN/HS.$fasta $methodEnc
    compEncDecDecompress $methodComp $dsPath/$FA/$VIRUSES/viruses.$fasta $methodEnc
    for i in 1 2; do
      compEncDecDecompress $methodComp $dsPath/$FA/$Synth/SynFA-$i.$fasta $methodEnc
    done
    ;;

  "fq" | "FQ" | "fastq" | "FASTQ") # FASTQ -- human - Denisova - synthetic
    for i in ERR013103_1 ERR015767_2 ERR031905_2 SRR442469_1 SRR707196_1; do
      compEncDecDecompress $methodComp $dsPath/$FQ/$HUMAN/$HUMAN-$i.$fastq $methodEnc
    done
    for i in B1087 B1088; do
      compEncDecDecompress $methodComp $dsPath/$FQ/$DENISOVA/$DENISOVA-${i}_SR.$fastq $methodEnc
    done
    for i in 1 2; do
      compEncDecDecompress $methodComp $dsPath/$FQ/$Synth/SynFQ-$i.$fastq $methodEnc
    done
    ;;
  esac

...
# Download
for d in "$ARCHAEA Archaea" "$BACTERIA Bacteria" "$FUNGI Fungi" "$PLANTS  Plants" \
  "$VIRUSES  Viruses"; do
  set $d
  ...
  cd $DS_PATH/$1
  cp "../../../$scripts_data_perl/Download$2.pl" \
  ...
  grep -Ev "^$" $smallCapsD.fa | ./goose-extractreadbypattern "complete genome" | ./goose-splitreads

...
if [[ ! -d $dataset/$FA/$HUMAN ]]; then
  mkdir -p $dataset/$FA/$HUMAN
fi

# Download and remove blank lines
for i in {1..22} X Y MT; do
  wget $WGET_OP $HUMAN_FA_URL/$HUMAN_CHROMOSOME$i.fa.gz
  gunzip <$HUMAN_CHROMOSOME$i.fa.gz | grep -Ev "^$" >$dataset/$FA/$HUMAN/$HUMAN-$i.$fasta
...
for dual in "alts AL" "unplaced UP" "unlocalized UL"; do
  ...
  gunzip <$HUMAN_CHR_PREFIX$1.fa.gz | grep -Ev "^$" >$HOME/$dataset/$FA/$HUMAN/$HUMAN-$2.$fasta

...
function run_redundancy_results {
  local CRYFA=1
  local MFCOMPRESS=1
  local DELIMINATE=1
  local DATA_SET="$ARCHAEA $BACTERIA $FUNGI $PLANTS $VIRUSES"
    ...
    for d in $DATA_SET; do
      for i in ../../$dataset/$redun/$d/*.$fasta; do
        ...
        printf "cryfa\t$d\t$inwf\t$origSize\t$compSize\t%s\n" \
          "$(echo "scale=5; ($compSize*8)/($origSize*2)" | bc -l)" >>../../$result/REDUN-CRYFA.$INF
        rm -f $i.cryfa
      done
    done
    cd ../..
    ...
    for d in $DATA_SET; do
      for i in ../../$dataset/$redun/$d/*.$fasta; do
        ...
        printf "MFCompress\t$d\t$inwf\t$origSize\t$compSize\t%s\n" \
          "$(echo "scale=5; ($compSize*8)/($origSize*2)" | bc -l)" >>../../$result/REDUN-MFCOMPRESS.$INF
        rm -f $i.mfc
      done
    done
    cd ../..
    ...
    for d in $DATA_SET; do
      for i in ../../$dataset/$redun/$d/*.$fasta; do
        ...
        printf "DELIMINATE\t$d\t$inwf\t$origSize\t$compSize\t%s\n" \
          "$(echo "scale=5; ($compSize*8)/($origSize*2)" | bc -l)" >>../../$result/REDUN-DELIMINATE.$INF
        rm -f $i.dlim
      done
    done
    cd ../..

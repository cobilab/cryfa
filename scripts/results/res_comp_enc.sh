...
function compEncDecDecompResOnDataset {
  ...
  for i in $ENC_METHODS; do
    ...
    for j in $FASTA_METHODS; do
      ...
      compEncDecDecompRes $j $i $FAdsPath/$VIRUSES/viruses.$fasta >>$OUT_FILE
      for k in 1 2; do
        compEncDecDecompRes $j $i $FAdsPath/$Synth/SynFA-${k}.$fasta >>$OUT_FILE
      done
    done
    ...
    for j in $FASTQ_METHODS; do
      for k in ERR013103_1 ERR015767_2 ERR031905_2 SRR442469_1 SRR707196_1; do
        compEncDecDecompRes $j $i $FQdsPath/$HUMAN/HS-${k}.$fastq >>$OUT_FILE
      done
      for k in B1087 B1088; do
        compEncDecDecompRes $j $i $FQdsPath/$DENISOVA/DS-${k}_SR.$fastq >>$OUT_FILE
      done
      for k in 1 2; do
        compEncDecDecompRes $j $i $FQdsPath/$Synth/SynFQ-${k}.$fastq >>$OUT_FILE
      done
    done
  done
}
...
#   Public API
#-------------------------------------------------------------------------------
function generate_compression_encryption_results {

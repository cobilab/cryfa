...
inline char frmt(const std::string& inFileName) {
  ...
  assert_single(!in.good(), std::format("failed opening \"{}\".", inFileName));
...
inline void show_help() {
  ...
  const std::string opt_space = std::format("{}     ", init_space);
  ...
  std::cerr << bold("NAME") << '\n'
            ...
            << '\n'
            << init_space << bold("-k") << " [" << underline("KEY_FILE") << "],  " << bold("--key")
            << " [" << underline("KEY_FILE") << "] \n"
            << opt_space << "key file name -- " << italic("MANDATORY") << '\n'
            << opt_space << "The KEY_FILE should contain a password. \n"
            << wrap_text(
                   "To make a strong password, the \"keygen\" program can be used via the command "
                   "\"./keygen\".",
                   opt_space)
            << '\n'
            << '\n'
            ...
            << wrap_text(
                   "Forces Cryfa not to compact, but shuffle and encrypt. If the input is "
                   "FASTA/FASTQ, it is considered as non-FASTA/FASTQ; so, compaction will be "
                   "ignored, but shuffling and encryption will be performed.",
                   opt_space)
            << '\n'
            << '\n'
            ...
            << '\n'
            << init_space << bold("-t") << " [" << underline("NUMBER") << "],  " << bold("--thread")
            << " [" << underline("NUMBER") << "] \n"
            ...
            << bold("Warning:") << ' '
            << wrap_text(
                   "the maximum file size supported is 64 GB. For larger files, you can split "
                   "them, e.g. by \"split\" command, and encrypt each chunk. After the decryption, "
 ...
 * @return Operation mode: 'c' for compress+encrypt or 'd' for decrypt+decompress
 */
char parse(Param& par, int argc, char** argv) {
  ...
  // Help
  if (exist(vArgs.begin(), vArgs.end(), "-h") || exist(vArgs.begin(), vArgs.end(), "--help")) {
    show_help();
  }

  // Version
  if (exist(vArgs.begin(), vArgs.end(), "--version")) {
    show_version();
  }
  ...
  if (file_size(par.in_file) > (1ull << 36)) {
    const std::string message = std::format(
        "Size of \"{}\" is larger than 64 GB. You can split it, e.g. by \"split\" command, and "
        "encrypt each chunk. After the decryption, you can concatenate the chunks, e.g. by \"cat\" "
        "command.",
        file_name(par.in_file));
    error(message);
  }

  // key -- MANDATORY
  assert_single(
      !exist(vArgs.begin(), vArgs.end(), "-k") && !exist(vArgs.begin(), vArgs.end(), "--key"),
  ...
  for (auto i = vArgs.begin(); i != vArgs.end(); ++i) {
    if (*i == "-v" || *i == "--verbose") {
      par.verbose = true;
    } else if ((*i == "-t" || *i == "--thread") && i + 1 != vArgs.end() && (*(i + 1))[0] != '-' &&
               is_number(*(i + 1)))
      par.n_threads = static_cast<byte>(stoi(*++i));
  }

  // Decrypt+decompress
  if (exist(vArgs.begin(), vArgs.end(), "-d") || exist(vArgs.begin(), vArgs.end(), "--dec")) {
    return 'd';
  }
  ...
  if (!exist(vArgs.begin(), vArgs.end(), "-f") && !exist(vArgs.begin(), vArgs.end(), "--force")) {

...
{
    pos_t p=(pos_t) (threadID*5);cerr<<p<<' ';
    ofstream of;
    of.open("hehe");
    of.seekp(p);
    
    mutx.lock();
    of<<"thr="<<(short)threadID;
    of.close();
    mutx.unlock();
...
//    ofstream pkfile;
//    pkfile.open(PK_FILENAME+to_string(threadID), std::ios_base::app);
//
    ...
    else if (headersLen <= MAX_C5 && qscoresLen > MAX_C5)
    {
        ...
        for (t = 0; t != n_threads; ++t)
        {
            ...
            if (c == (char) 253)
            {
                ...
                arrThread[t] = thread(&EnDecrypto::unpackHSQL, this, in.tellg(),
                           offset, hdrUnpack, XChar_qs, qsUnpack, t, unpackHdr);
    ...
    else if (headersLen > MAX_C5 && qscoresLen <= MAX_C5)
    {
        ...
        for (t = 0; t != n_threads; ++t)
        {
            ...
            if (c == (char) 253)
            {
                ...
                arrThread[t] = thread(&EnDecrypto::unpackHLQS, this, in.tellg(),
                           offset, XChar_hdr, hdrUnpack, qsUnpack, t, unpackQS);
